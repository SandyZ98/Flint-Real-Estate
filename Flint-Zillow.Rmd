...
library(foreign)
library(XML)
>>>>>>> d0392df45d33d78d9f0176c437d917abf55607df
```
...
>>>>>>> 5e0c636df6220c5f4385e2aa2d9e3fc7d92909b5
...
flint_acs5_series %>% gather(6,8:10,15:18,key = "type", value = "population") %>% 
  arrange(.,tract) %>% 
  filter(type %in% c("m_under5","f_under5","type","population")) %>% group_by(year, type) %>% 
...
flint_acs5_series %>% 
  ggplot(., aes(x=year, y=homevalue)) + geom_point() + geom_boxplot()
```

* ACS home values over time
>>>>>>> 5e0c636df6220c5f4385e2aa2d9e3fc7d92909b5
...
Then we set up the loop. The data extracted are turned into tibbles and they are merged into a final tibble. Daily data are saved for insurance.
```{r}
...
Then we do some data check. We finally gathered 9351 records in a tibble with 27 variables. We use the Zillow property ID(ZPID) to remove duplicates. Then we get 5843 unique records. 
```{r}
...
Some data cleaning steps to select only unique records in the dataset, covert long and lat to numeric, and convert dates from character to date format.
```{r}
#Keeping unique records of the dat
zillow <- unique.data.frame(table_zlw) 

#Changing longitude and latitude as numeric.
table_zlw$lat %<>% as.numeric()
table_zlw$long %<>% as.numeric()
...
Checking to see how many records we have per year
```{r}
years

zillow %>% 
  filter(lastSoldYear %in% years | taxAssess_year %in% years) %>%
  group_by(lastSoldYear)%>% 
  summarise(n(), mean(na.omit(taxAssessment)),
  mean(na.omit(zestimate)))
#70 records
...
#Boxplot
zillow_analysis %>% 
  ggplot(., aes(x=taxAssess_year, y=taxAssessment)) + geom_point() + geom_boxplot()

#Line graph
zillow_analysis %>% 
  mutate(year = as.numeric(taxAssess_year)) %>%
  ggplot(., aes(x=year, y=taxAssessment)) + geom_line()

#Home sale records
#Boxplot
zillow_analysis %>% 
  ggplot(., aes(x=lastSoldYear, y=lastSoldPrice)) + geom_point() + geom_boxplot()

#Line graph
zillow_analysis %>% 
  mutate(year = as.numeric(lastSoldYear)) %>%
  ggplot(., aes(x=year, y=taxAssessment)) + geom_line()
```
...
What is the average discrepancy between reported values and administrative records?
...
We map the data to see if we need more addresses per census tract.

```{r}
flint_map <- map_data("county", region = "michigan")
...
table_zlw %>%
  qmplot(long, lat, data = ., maptype = "toner-lite", size = I(0.75), alpha = I(0.75))
```
