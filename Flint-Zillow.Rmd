...
The Flint water crisis began in 2014, after the water supply of 100,000 people was switched from the Detroit River to the Flint River. Because water was not properly treated following the switch, the city's population was exposed to elevated lead levels.

Previous studies have explored the effects of the water crisis on children’s lead levels (Hanna-Attisha, 2016) and depopulation in the city due to perceptions of unsafe water (Morckel and Greg Rybarczyk, 2018).

This project aims to analyse the effect of the Flint water crisis on residential home values. We use data from the American Community Survey (ACS) and the Zillow real estate API to examine housing trends over time.

While the ACS asks respondents to give an estimate of their home value ("About how much do you think this house and lot, appartment or mobile home (and lot, if owned) would sell for if it were for sale?"), we hypothesize that this survey item will be affected by measurement error. Respondents may likely not be able to give an accurate and unbiased estimate of their home's value.

We will therefore use alternative sources of data to calculate home values and compare these to the ACS on a tract level. Our research questions are as follows:
...
We obtain a key for the Census bureau API and save this as an R object.
...
From the listCensusMetadata command, we obtain the variables of interest:
...
>>>>>>> 23733c050f615c097f16918b438e82aba3af3269
...
Now, we link the Zillow addresses to census tracts based on the provided latitude and longitude and the tigris package.
...
zillow_analysis$tract <- zillow_analysis$geo %>% 
  substr(7,11) %>% #subsetting string
  as.numeric()
```

Now we perform an inner join on the Zillow and ACS data. 
...
### Measure lead pipe coverage at the tract level
...
pipe_coverage<-pipe_raw %>% 
  select(STATEFP,COUNTYFP,TRACTCE,GEOID,NAME,NAMELSAD,TRACTA,Area,Tract_area) %>% 
  group_by(TRACTCE) %>% 
  summarise(sum_pipe_area=sum(Area),Tract_area=unique()) %>% 
  mutate(coverage=sum_pipe_area/Tract_area)
tract_pipe_coverage <- aggregate(TRACTCE~coverage,data=pipe_coverage,FUN=mean)
```
...
Now visualizations! Here, we map the property addresses from Zillow.
...
ggplot(data = merged_geo) +
  ...
  theme_light() +
    labs(title = "Addresses gathered from Zillow API (n=5,843)")
```
...
We will visualize the change in home values from 2013 to 2016, roughly when the Flint water crisis became public.

```{r}
rate_of_change_housing <- acs_geo %>%
  mutate(home_change_rate_1316 = (homevalue_2016 - homevalue_2013)/homevalue_2013) %>% 
  merge(pipe_coverage,by="tract") %>% 
  ggplot() +
  geom_sf(aes(fill=home_change_rate_1316)) +
  scale_fill_distiller(palette = "RdYlGn", direction=1) +
  theme_light() +
  labs(title = "Flint: Rate of change in home values 2013-2016",
       caption="Source: American Community Survey",
       fill="Change in dollars")
rate_of_change_housing
```

Here are the coverage rate of lead-pipe-influence area for each census tract.
```{r}
Lead_pipe_coverage<-acs_geo %>%
  merge(pipe_coverage,by="tract") %>% 
  ggplot() +
  geom_sf(aes(fill=coverage)) +
  scale_fill_distiller(palette = "PuBu", direction=1) +
  theme_light() +
  labs(title = "Flint: Coverage rate of lead-pipe-influence area 2013-2016",
       caption="Source: American Community Survey",
...
We observed that home values decreased more in areas with a higher percentage of black-only households.