...
flint_geo %>%
  filter(GEOID %in% unique(merged$geo))
```

Merging geometry with ACS/Zillow dataset.
...
Now visualizations!
```{r}
st_geometry(flint_geo) %>% plot()

ggplot() +
  geom_sf(data = flint_geo, aes(fill = events), size = 0.5) +
...
leaflet() %>%
  addTiles() %>%
  addCircles(merged, lng= merged$long, lat = merged$lat) %>%
  addPolygons(flint_geo)
```
